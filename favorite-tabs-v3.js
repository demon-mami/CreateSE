(() => {
  ...
  // Remove the v2 dynamically-generated tab row. v3 owns the visible switcher.
  dialog.querySelectorAll('.favorite-collection-tabs').forEach(node => node.remove());

  const tabs = document.createElement('div');
  tabs.id = 'favoriteManagerTabs';
  tabs.className = 'favorite-manager-tabs';
  tabs.setAttribute('role', 'tablist');
  tabs.setAttribute('aria-label', 'Presetとお気に入りの切り替え');
  const actions = dialog.querySelector('.favorite-dialog-actions');
  if (actions) actions.insertAdjacentElement('afterend', tabs);
  else dialog.insertBefore(tabs, list);
  ...
  function renderTabs() {
    ...
    const specs = [
      ['preset', `Preset ${presets.length || 12}`],
      ['favorite1', `お気に入り1 ${collections.favorite1.length}/${MAX_ITEMS}`],
      ['favorite2', `お気に入り2 ${collections.favorite2.length}/${MAX_ITEMS}`],
    ];

    tabs.replaceChildren();
    for (const [key, text] of specs) {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'favorite-manager-tab';
      button.setAttribute('role', 'tab');
      button.setAttribute('aria-selected', activeTab === key ? 'true' : 'false');
  ...
  function renderList() {
    ...
    if (activeTab === 'preset') {
      const presets = readPresets();
      if (title) title.textContent = 'Preset 12';
    ...
    if (title) title.textContent = `${slotLabel(activeTab)} ${items.length}/${MAX_ITEMS}`;
    ...
    items.forEach((entry, index) => {
      list.append(makePairButton(entry, `${index + 1}.`, activeTab));
    });
  }
  ...
  style.dataset.feature = 'favorite-manager-v3';
  style.textContent = `
    .favorite-manager-tabs{
      display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0 0 12px;
    }
    .favorite-manager-tab{
      min-height:46px;padding:0 8px;border:1px solid rgba(255,255,255,.20);border-radius:10px;
      background:rgba(43,58,65,.88);color:#fff;font-size:11px;font-weight:850;touch-action:manipulation;
    }
    .favorite-manager-tab[aria-selected="true"]{
      border-color:rgba(121,212,236,.90);background:rgba(68,149,174,.48);color:#f2fdff;
      box-shadow:0 0 0 1px rgba(121,212,236,.18),0 0 14px rgba(75,232,255,.10);
    }
    ...
    @media(max-width:430px){
      .favorite-manager-tabs{gap:5px}
      .favorite-manager-tab{font-size:9px;padding-inline:4px}
    }
  `;
